...
test_that("combining predictions", {
  ...
  expect_false(ncol(preds[[1L]]$data$cif$`2`) == ncol(preds[[2L]]$data$cif$`2`))
  expect_false(ncol(preds[[2L]]$data$cif$`2`) == ncol(preds[[3L]]$data$cif$`2`))
...
test_that("as_prediction_cmprsk", {
  p = lrn("cmprsk.crr")$train(task)$predict(task)
